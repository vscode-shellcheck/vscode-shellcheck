...
async function getFixAllCodeAction(document: vscode.TextDocument): Promise<vscode.CodeAction | undefined> {
    ...
    for (const range of uniqueRanges) {
        const codeActionsForDiagnostic = await executeCodeActionProvider(document.uri, range);

        if (codeActionsForDiagnostic) {
            // get only codeactions which perform edits
            const actionToFix = codeActionsForDiagnostic.filter((action) => action.edit);
            codeActions.push(...actionToFix);
        }
    }

    if (codeActions.length > 0) {
        const fixAll = new vscode.CodeAction('Fix All ShellCheck', FixAllProvider.fixAllCodeActionKind);
...
export class FixAllProvider implements vscode.CodeActionProvider {
    public static readonly fixAllCodeActionKind = vscode.CodeActionKind.SourceFixAll.append('shellcheck');
