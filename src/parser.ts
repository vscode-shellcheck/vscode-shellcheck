...
class JsonParserMixin {
    ...
    protected doParse(textDocument: vscode.TextDocument, problems: ShellCheckProblem[]): ParseResult[] {
        ...
        for (const problem of problems) {
            ...
            const codeAction = this.options?.enableQuickFix ? this.makeCodeAction(problem, textDocument, diagnostic) : null;
            result.push({
                diagnostic,
                codeAction
            });
    ...
    protected makeCodeAction(problem: ShellCheckProblem, textDocument: vscode.TextDocument, diagnostic: vscode.Diagnostic): vscode.CodeAction | null {
        ...
        const fix = new vscode.CodeAction(`Apply fix for SC${problem.code}`, vscode.CodeActionKind.QuickFix);
    ...
    protected makeDiagnostic(problem: ShellCheckProblem): vscode.Diagnostic {
        ...
        const endCharacter = problem.endColumn ? problem.endColumn - 1 : startPos.character;
...
class JsonParser extends JsonParserMixin implements Parser {
    ...
    constructor(public readonly textDocument: vscode.TextDocument, options?: ParserOptions) {
        super(options);
    }
...
export function createParser(textDocument: vscode.TextDocument, options?: ParserOptions): Parser {
    if (options && options.toolVersion && semver.gte(options.toolVersion, BEST_TOOL_VERSION)) {
